...
int lookup(struct node *root, int target) {
	if (root == NULL)
		...
		if (target < root->data) 
			return (lookup(root->lesser, target)) ;
		else 
			return (lookup(root->higher, target)) ;
	}
}
...
struct node *insertNode(struct node *curr, int data) {
	if (curr == NULL)
		...
		if (data == curr->data)
			...
			if (data < curr->data)
				curr->lesser = insertNode(curr->lesser, data) ;
			else 
				curr->higher = insertNode(curr->higher, data) ;
		}
		return (curr) ;
	}
}
	...
	*/
	if (value == curr->data) {
		if (curr->lesser != NULL && curr->higher != NULL) {
			temp = minValue(curr->higher) ;
			curr->data = temp->data ;
			/* Delete the candidate node and re-assign the subtree
			* to which it belongs. 
			*/
			curr->higher = deleteNode(curr->higher, curr->data) ;
		}
		else {
			temp = curr ;
			if (curr->lesser == NULL)
...
int bstSize(struct node *curr) {
	if (curr == NULL)
		...
		return (bstSize(curr->lesser)+1+bstSize(curr->higher)) ;
}
...
char* printTree(struct node *curr) {
	...
	char cs[10] ;
	strcpy(ss, printTree(curr->lesser)) ;
	...
	ss = strcat(ss, printTree(curr->higher)) ;
	return ss ;
}
